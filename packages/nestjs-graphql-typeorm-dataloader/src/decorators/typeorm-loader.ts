...
export function TypeormLoader<V> (typeFunc: (type?: void) => ObjectType<V>, keyFunc: KeyFunc, option?: TypeormLoaderOption): PropertyDecorator
...
export function TypeormLoader<V> (
  ...
  const getArgs = (): [KeyFunc, TypeormLoaderOption | undefined] => {
    return option != null || typeof keyFuncOrOption == 'function' ? [ keyFuncOrOption as KeyFunc, option ] : [ typeFuncOrKeyFunc as KeyFunc, keyFuncOrOption as TypeormLoaderOption ]
  }
  return TypeormLoaderImpl<V>(...getArgs())
}

function TypeormLoaderImpl<V> (keyFunc: KeyFunc, option?: TypeormLoaderOption): PropertyDecorator {
  return (target: Record<string, any>, propertyKey: string | symbol): void => {
    UseMiddleware(async ({ root, context }, next) => {
      const tgdContext = context._tgdContext as Context
      if (tgdContext.typeormGetConnection == null) {
        throw Error('typeormGetConnection is not set')
      }
      const relation = tgdContext.typeormGetConnection().getMetadata(target.constructor).findRelationWithPropertyPath(propertyKey.toString())
...
async function handler<V> (
  { requestId, typeormGetConnection }: Context,
  ...
  if (typeormGetConnection == null) {
    throw Error('Typeorm connection is not available.')
  }
...
async function handleToMany<V> (foreignKeyFunc: (root: any) => any | undefined, root: any, tgdContext: Context, relation: RelationMetadata): Promise<any> {
  return handler(
    tgdContext,
    relation,
...
async function handleToOne<V> (foreignKeyFunc: (root: any) => any | undefined, root: any, tgdContext: Context, relation: RelationMetadata): Promise<any> {
  return handler(
    tgdContext,
    relation,
    relation.inverseEntityMetadata.primaryColumns,
...
async function handleOneToManyWithSelfKey<V> (selfKeyFunc: (root: any) => any | any[], root: any, tgdContext: Context, relation: RelationMetadata): Promise<any> {
  return handler(
    tgdContext,
    relation,
...
async function handleOneToOneNotOwnerWithSelfKey<V> (selfKeyFunc: (root: any) => any | undefined, root: any, tgdContext: Context, relation: RelationMetadata): Promise<any> {
  return handler(
    tgdContext,
    relation,
    relation.entityMetadata.primaryColumns,
...
function directLoader<V> (relation: RelationMetadata, connection: Connection, grouper: string | ((entity: V) => any)) {
...
class SelfKeyDataloader<V> extends DataLoader<any, V[]> {
  constructor (relation: RelationMetadata, connection: Connection, selfKeyFunc: (root: any) => any) {
    super(async (ids) => {
