...
export async function setEnvironmentVariables(packageFile = join(process.cwd(), 'package.json')): Promise<void> {
  // this should cover many cases and different package managers with various fallbacks
  const name =
    process.env.PACKAGE_NAME ??
    process.env.npm_package_name ??
    (basename(process.env.PWD ?? '').length ? basename(process.env.PWD) : undefined)
  const version = process.env.PACKAGE_VERSION ?? process.env.npm_package_version

  let packageJson: Record<PropertyKey, any>

  // only read package file if we miss something
  if (!(name && version)) {
    packageJson = JSON.parse(await readFile(packageFile, { encoding: 'utf8' }))
  }
